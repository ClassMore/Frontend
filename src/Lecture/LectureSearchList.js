      ...
      {Array.isArray(lectures) && lectures.map(lecture => (lecture._source.imageLink &&
        < div className="products-row search" onClick={() => handleOnclick(lecture)} style={{caretColor: 'transparent'}}>
          ...
          {lecture._source.salePrice !== lecture._source.ordinaryPrice &&
            <div className="product-cell sales"><span className="cell-label">SalePrice:</span>{lecture._source.salePrice.toLocaleString('ko-KR')}원</div>
          }
          <div className="product-cell price"><span className="cell-label">Price:</span>
            {lecture._source.ordinaryPrice === 0 ? '무료' : lecture._source.ordinaryPrice.toLocaleString('ko-KR') + '원'}
          </div>
        </div >
        ))}
    </>

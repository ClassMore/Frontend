...
const Lecture = () => {
  ...
  const getLectures = useCallback(async () => {
    ...
    try{
      await axios({
        url: `${url}lecture?page=${page}&size=10`,
        method: 'GET'
      }).then((res) => {
        // console.log(res);
  ...
  const handleOnclick = (lectures) => {
    nav(`/lecture/detail/${lectures.lectureId}`, {state: lectures.lectureId});
  }
     ...
     <div className="apps-card">
      {
          Array.isArray(lectures) && lectures.map((lecture, idx) => (
            ...
            {
              lectures.length - 1 === idx ? 
              (
                
              <div className="app-card" style={{marginTop: '2vh', marginLeft: '0.5vw'}}>
              <tr id={idx} key={lecture.id} onClick= {() => handleOnclick(lecture)} ref={ref}>
                <td>
                  <img src={lecture.imageLink} style={{opacity:"0.8", borderRadius:"10px" , height:'100%'}}/>
                </td>
                <div className="app-card__subtext">
                  <span>강의명: {lecture.title}</span>
                  <span>원가: {lecture.ordinaryPrice}</span>
                  <span>할인가: {lecture.salePrice}</span>
                </div>
              </tr>
              </div>
            ) : 
            ...
            <div className="app-card" style={{marginTop: '2vh', marginLeft: '0.5vw'}}>
              <tr id={idx} key={lecture.id} onClick= {() => handleOnclick(lecture)}>
                <td>
                 <img src={lecture.imageLink} style={{opacity:"0.8", borderRadius:"10px" , height:'100%'}}/>
                </td>
                <div className="app-card__subtext">
